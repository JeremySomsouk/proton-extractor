    ...
    match args.format {
        ...
        OutputFormat::Html => {
            ...
            for ((year, _month), summary) in &grouped {
                ...
                writeln!(out_writer, "    <h2>📅 {} {}</h2>", summary.month_name, year)?;
                writeln!(out_writer, "    <ul class=\"event-list\">")?;
                if !args.quiet && !args.sum_only {
                    for event in &summary.events {
                        if let Some(mins) = event_duration_minutes(event) {
                            writeln!(out_writer, "      <li class=\"event-item\">")?;
                            writeln!(out_writer, "        <span class=\"summary\">{}</span>", html_escape(&event.summary))?;
                            writeln!(out_writer, "        <span class=\"duration\">{}</span>", format_hours(mins))?;
                            writeln!(out_writer, "      </li>")?;
                        }
                    }
                }
                writeln!(out_writer, "    </ul>")?;
                writeln!(out_writer, "    <div class=\"person-breakdown\">")?;
                writeln!(out_writer, "      <h3>👤 By Person</h3>")?;
                ...
                writeln!(out_writer, "    </div>")?;
                writeln!(out_writer, "    <div class=\"total\">")?;
                writeln!(out_writer, "      📊 Total: {} ({:.1}%)", format_hours(month_total), (month_total as f64 / grand_total_minutes as f64) * 100.0)?;
                writeln!(out_writer, "    </div>")?;
                writeln!(out_writer, "  </div>")?;
            }
